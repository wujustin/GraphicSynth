...
class RenderFont(object):
    ...
    def render_multiline(self,font,text):
        ...
        line_bounds = font.get_rect(lines[np.argmax(lengths)].strip())
...
class FontState(object):
    ...
    def get_aspect_ratio(self, font, size=None):
        ...
        try:
            ...
            # WG: fix a bug: divide by zero
            r = np.abs(cutils.np_float_div(sizes[:,1], sizes[:,0]))
...
class TextSource(object):
    ...
    def __init__(self, min_nchar, fn):
        ...
        for filename in files:
            ...
            text_lines = cutils.loadfile(fc)
            self.txt.extend(text_lines)
#             with open(fc, 'r', encoding='utf8') as f:
            ...
            for i in range(nline):
                ...
                while len(lines[i]) > nchar_max: #chop-off characters from end:
                    if not np.any([ch.isspace() for ch in lines[i]]):
                        ...
                        #lines[i] = lines[i][:len(lines[i])-lines[i][::-1].find(u'\u3000')].strip()
